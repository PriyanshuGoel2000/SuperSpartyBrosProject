...
public class CharacterController2D : MonoBehaviour {
	...
	void Update()
	{
		...
		//allow double jump when grounded
		if (_isGrounded)
		{
			_canDoubleJump = true;
		...
		else if (_canDoubleJump && Input.GetButtonDown("Jump"))//if can double jump and pressed jump button allow double jump
		{
			DoJump();
			//disabling double jump as double jump can only be done once
